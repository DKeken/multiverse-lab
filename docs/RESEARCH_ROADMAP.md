...
Track 3 activation (stochastic gravitational-wave background): feasibility anchored to the NANOGrav 15 yr primary result (`A = 2.4^{+0.7}_{-0.6}×10⁻¹⁵` at `1/yr`, `f^{-2/3}`, HD correlations `3.5–4σ`; arXiv:2306.16213). Closed-form SMBHB spectrum (`Ω(1/yr)=7.9×10⁻⁹`) versus causal first-order-phase-transition templates gives `33/60` grid points passing today-`2σ`/twenty-year-`3σ` discrimination → **chain gate approved**. Next steps, in order: freeze a registry entry for the public NANOGrav chains repository (URL/SHA-256/size/license), then run noise-marginalized model comparison using the analytic-null covariance machinery exported from this session. The multiverse bridge: bubble-collision physics now has two independent observational channels — CMB imprint (closed at `5°`) and early-universe stochastic background (open). Temperature-bridge formula is order-of-magnitude `[INFERENCE]` until verified.

KDE-discrimination verdict (first GW data result): scoring closed-form spectra against the NANOGrav free-spectrum KDE marginals (variant `30f_fs{hd}`, 30 bins, source-verified convention `log10rho = ½log10 S_r`, `S_r = A²/(12π²)·f_yr^{γ−3}f^{−γ}/Tspan`; PT via convention-free `Ω`-ratio) validates the pipeline — published SMBHB median sits inside 90% support in `28/30` bins — and delivers the constraint: causal bubble-collision templates are preferred at only **`2/45`** extreme-corner points (`f_pk=10⁻⁷ Hz`, `n₂=+1`, `Ω_pk=10⁻⁷`). The cheap marginal screen is complete; proper bubble-collision model comparison requires the declared `pt_bubble` MCMC chains (22.4 GB, registry-frozen) or next-generation PTA data. Implementation correction is disclosed: an initial run misread log-PDF rows as linear densities and was discarded before any provenance claim.
