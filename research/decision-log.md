...
- First observational screen on development data: analytic multi-radius matched filter on WMAP ILC T (|b|>=20 deg, pseudo-Cl, cosmic-variance covariance) finds no excess at 10/15/20/30 degrees; all pre-trials z within plus-minus one. The post-trials column subtracts the lobe penalty from signed z and is only meaningful for positive excursions; nothing approaches it, so the no-detection conclusion is robust to that definition.
- The screen delivers 95 percent upper limits R_0^L of 1.09e-4 to 2.23e-4, sitting 4.4 to 29 times below typical physical amplitudes at 15 to 30 degrees. Any future excess claim must pass a frozen end-to-end null pipeline before promotion language.
- End-to-end null calibration closes the development-data screen: 1000 identical-pipeline Gaussian skies give observed-maximum p = 0.417 (null mean 0.848, max 4.081), and empirical 95 percent limits R_0^L of 1.08e-4 to 2.29e-4 reproduce the analytic screen within a few percent. The bubble-collision search on available public temperature data is complete; further observational work requires Planck approval or a different observable.
- Analytic-null invention status: the closed-form covariance (Legendre moments of masked kernels plus the pixel window) reproduces fresh-null correlations to 0.027 and gives p 0.4022 against the published 0.4166 — Monte Carlo is deletable for this statistic class. The frozen five-percent diagonal check formally fails because the reference thousand-null sigma carries its own 2.2-percent Monte Carlo error and the worst radius deviates by 5.26 percent (about 2.3 sigma of that error); the artifact therefore stays INVALID, thresholds are not retroactively adjusted, and any future use should propagate reference uncertainty into the tolerance by a new preregistered amendment.
- Track 3 opens by feasibility verdict: closed-form SMBHB and first-order-phase-transition spectra anchored to arXiv:2306.16213 show 33 of 60 grid points distinguishable at today 2 sigma rising past 3 sigma with 20-year data. Decision: proceed to a registry-frozen public-chains fetch gate; the same collision physics now has two observational channels (CMB imprint closed at five degrees, stochastic background open). The analytic-null method from this session transfers directly to Gaussian pulsar-timing statistics.
